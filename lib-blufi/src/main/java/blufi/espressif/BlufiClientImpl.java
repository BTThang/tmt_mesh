...
import blufi.espressif.response.BlufiCustomResult;
...
//            int totalLen = dataBytes[0] | (dataBytes[1] << 8);
    ...
    private void parseWifiScanList(byte[] data) {
        ...
        while (dataReader.available() > 0) {
            ...
            int bssidRead = dataReader.read(bssid, 0, 6);
            if (bssidRead != 6) {
                Log.w(TAG, "Sai do dai BSSID");
                break;
            }

            byte[] ssid = new byte[33];
            int ssidRead = dataReader.read(ssid, 0, 33);
            if (ssidRead != 33) {
                Log.w(TAG, "Sai do dai SSID");
                break;
            }
            ...
            String Namessid = new String(ssid);
            sr.setSsid(Namessid);
            sr.setRssi(rssi);
            result.add(sr);
...
//            System.out.print("Gia tri cua tung chuoi========"+result.get(i) + "\n");
//        }

        System.out.println(result);
//        System.out.println("Kich thuoc ============" + result.size());
//        System.out.println("File dau tien============" + ((LinkedList<BlufiScanResult>) result).getFirst());
...
//    onDeviceScanResult(BlufiCallback.STATUS_SUCCESS, result);
    ...
    private void parseWifiScanCustomList(byte[] data) {
        List<BlufiCustomResult> result = new LinkedList<>();
...
//            byte Bssid = (byte) dataReader.read();
            byte rssi = (byte) dataReader.read();
            byte[] ssidBytes = new byte[length - 1];
            int ssidRead = dataReader.read(ssidBytes, 0, ssidBytes.length);
            if (ssidRead != ssidBytes.length) {
            ...
            BlufiCustomResult sr = new BlufiCustomResult();
            sr.setmType(BlufiCustomResult.TYPE_WIFI);
            sr.setmRssi(rssi);
            String ssid = new String(ssidBytes);
            sr.setmSsid(ssid);
            result.add(sr);
        ...
        onDeviceCustomResult(BlufiCallback.STATUS_SUCCESS, result);
    }


    private void onDeviceCustomResult(final int status, final List<BlufiCustomResult> results) {
        mUIHandler.post(() -> {
            if (mUserBlufiCallback != null) {
                mUserBlufiCallback.onDeviceCustomResult(mClient, status, results);
            }
        });
...
//===============================================================================
    ...
    //    Add by user
//    private void onReceiveCustomScanData(final byte[] data) {
//        mUIHandler.post(() -> {
//            if (mUserBlufiCallback != null) {
//                int status = BlufiCallback.STATUS_SUCCESS;
//                mUserBlufiCallback.onDeviceCustomResult(mClient, status, data);
//            }
//        });
//    }
